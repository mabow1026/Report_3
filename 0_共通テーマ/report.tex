...
VSCodeの拡張機能のdraw.ioを用いてRLC直列回路の実験回路図を作成した。これを図\ref{fig:RLC_直列}に示す。
また、RLC並列回路の場合の実験回路図を\ref{fig:RLC並列}に示す。
\begin{figure}[htbp]
...
\ref{sec:RLC直列}で行った実験の結果を整理し、インピーダンス及び偏角を算出した。
また、算出した値をグラフにしたものをそれぞれ図\ref{fig:RLC直列_インピーダンス}と図\ref{fig:RLC直列_偏角}に示す。
さらに、インピーダンスの軌跡を複素平面に描画したものを図\ref{fig:RLC_軌跡１}に示す。
\begin{figure}[htbp]
	\centering
	\includegraphics[keepaspectratio, scale=0.5]{RLC直列インピーダンス.png}
...
\begin{figure}[htbp]
	\begin{minipage}[b]{0.45\linewidth}
		\centering
		\includegraphics[keepaspectratio, scale=0.5]{RLC直列偏角.png}
		\caption{RLC直列回路の偏角}
		\label{fig:RLC直列_偏角}
	\end{minipage}
	\begin{minipage}[b]{0.45\linewidth}
		\centering
		\includegraphics[keepaspectratio, scale=0.5]{RLC直列軌跡.png}
...
\ref{sec:RLC並列}で行った実験の結果を整理し、インピーダンス及び偏角を算出した。
また、算出した値をグラフにしたものをそれぞれ図\ref{fig:RLC並列_インピーダンス}と図\ref{fig:RLC並列_偏角}に示す。
さらに、インピーダンスの軌跡を複素平面に描画したものを図\ref{fig:RLC_軌跡２}に示す。
\begin{figure}[htbp]
	\centering
	\includegraphics[keepaspectratio, scale=0.5]{RLC並列インピーダンス.png}
...
\begin{figure}[htbp]
	\begin{minipage}[b]{0.45\linewidth}
		\centering
		\includegraphics[keepaspectratio, scale=0.5]{RLC並列偏角.png}
		\caption{RLC並列回路の偏角}
		\label{fig:RLC並列_偏角}
	\end{minipage}
	\begin{minipage}[b]{0.45\linewidth}
		\centering
		\includegraphics[keepaspectratio, scale=0.6]{RLC並列軌跡.png}
