...
N個すべてのデータが同じであると仮定したとき、平均値は平均値はN個のデータそれぞれの値と等しくなる。
...
よって、理想の平均値はあるデータ群のうちの一つのデータと等しい値で、理想の標準偏差は０であると考える。
...
理想的には出力電圧と入力電圧が等しくなると考えられる。よって傾き１で切片０の$y=x+0$となる。
...
\begin{equation}
	...
	\label{eq:ohm}
\end{equation}
式\ref{eq:ohm}を変形し、電圧電流特性の傾きをaとしたときに次のように表せる。
\begin{align}
	a & = \frac{1}{R\, [\Omega]}           \\
	\label{eq:henkei1}
	a & = \frac{1}{R\, [\mathrm{k}\Omega]} \\
	\label{eq:henkei2}
\end{align}
式(\ref{eq:henkei1})、式(\ref{eq:henkei2})を用いて傾きを求め、その値から抵抗値の計算値と誤差を導出し整理したものを表\ref{tab:gosa}に表す。
式(\ref{eq:hankei1})は横軸V、縦軸Aのときに成り立つ式であり、式(\ref{eq:henkei2})は横軸V、縦軸maのときに成り立つ式である。
\begin{table}[htbp]
	\centering
	\caption{抵抗の誤差率}
	\begin{tabular}{c|ccc}
...
\subsection{可変抵抗の抵抗値}
式(\ref{eq:henkei1})や式(\ref{eq:henkei2})で示したように、電圧電流特性の傾きは抵抗値に依存している。
