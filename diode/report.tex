...
\begin{enumerate}[i)]
	\item 直流電圧電流特性\\
	      直流電圧電流特性を図\ref{fig:tokusei}に示す。この図は図\ref{fig:d2}に$E_\mathrm{Vp}$を$0.8\,\mathrm{V}$としたときの負荷線を加えたものである。
...
\begin{enumerate}[i)]
	...
	\item $V_\mathrm{L} - I_\mathrm{Z}$特性\\
	      ...
	      \begin{figure}[H]
		      \centering
		      \includegraphics[keepaspectratio, scale=0.5]{ZD2.pdf}
		      \caption{$ツェナーダイオードの電流電圧特性$}
		      \label{fig:zd2}
	      \end{figure}
	\item	入出力特性の考察\\
...
\subsubsection{結果}
計測した電圧$V_\mathrm{L}$と電流$I_\mathrm{L}$を表\ref{tab:const}に示す。また，この値をグラフに描画したものを図\ref{fig:const1}，図\ref{fig:const2}に示す。
\begin{figure}[H]
	\centering
...
\begin{enumerate}[i)]
	...
	\item	 ライトを一つのみ点灯させた場合と，二つ点灯させた場合の両方で太陽電池の四つ角と中央の照度を測定した。
...
\subsubsection{結果}
ライトの数と照度の関係を表\ref{tab:light}に示す。照度は，太陽電池の4つ角と中央の照度の平均値を代表値として用いた。
...
\begin{enumerate}[i)]
	\item 各条件下における電流電圧特性の測定結果を用いた電力の算出\\
	      表\ref{tab:taiyo}から電力を算出し，その値をグラフに描画したものを図\ref{fig:taiyo2}に示す。また，核実験条件における最大電力点を表に整理したものを表\ref{tab:max}に示す。
	      表\ref{tab:max}と図\ref{fig:taiyo2}より，目標温度が$30\,\mathrm{^\circ C}$の場合はライトの数に関わらず$18\,\mathrm{V}$付近が電力が最も大きいことがわかる。
	      また，目標温度が$50\,\mathrm{^\circ C}$の場合は，ライトの数に関わらず$16\,\mathrm{V}$付近が最も電力が大きいことがわかる。
	      以上の事実より，太陽電池の温度が低いかつ照度が高い場合に最大電力は大きくなり，温度が高いかつ照度が低い場合に最大電力が小さくなることがわかった。\\
	      太陽電池は，太陽の光エネルギーを吸収して直接電気に変えるエネルギー変換素子である\cite{taiyo}。
	      シリコンや化合物などの半導体で作られており、この半導体に光が当たると、日射強度に比例して発電する\cite{taiyo}。
	      シリコンなどの半導体の温度が上昇すると，結晶格子間隔（格子定数）が伸びることでバンドギャップ（禁制帯幅）が狭くなる。バンドギャップが狭くなると出力電圧が低下し，発電効率が落ちてしまう。\cite{ondo}。
	      このことから，目標温度がが高い場合は同じ照度でも最大電力が小さかったと考えることができる。
	      \begin{figure}[H]
		      \centering
	      ...
	      \begin{table}[H]
		      ...
		      \label{tab:max}
	      \end{table}
	\item MPPT制御が必要とされる理由\\
	      MPPT制御とは，和名に直すと最大電力点追従制御である。名前の通り，気象条件で変化する気温や照度に合わせて最適な動作点に追従する制御のことである。
	      このMPPT制御のアルゴリズムには，山登り法と電圧追従法の二通りがあり，次に紹介する。\\
	      山登り法は，電圧を一方向（増加または減少）に変化させていき、電力が増加から減少に転換すると電圧を変化させる方向を逆方向にする。これを繰り返すことにより、常に電力が最大となる最適動作点に制御する方式である\cite{mppt1}。\\
	      電圧追従法は、予め太陽電池のP-V特性より最大点が得られる時の太陽電池の出力電圧を求めておき、その電圧値になるように、昇圧コンバータにPWM制御を行なう\cite{mppt2}。
	      そのため，電力を求める必要が無く計算量が少ないため，マイクロコントローラの動作周波数を下げることができ，大幅に消費電力を低減することができる\cite{mppt2}。\\
	      これらの制御を行うことで，変化する気象条件下においても安定した出力を得ることができる。
	      前述した通り，電圧追従法は制御回路も単純であり，マイクロコントローラーの計算量も少ないため，低コストかつ省電力で動作できることから，一般的にはこちらが用いられるようである。
\end{enumerate}
...
\begin{thebibliography}{9}%参考文献数が10以上の場合は9を99に変更
	...
	\bibitem{zd} Semiconductor Components Industried LLC, ``1N4728A - 1N4758A Zener Diodes'', \url{https://rocelec.widen.net/view/pdf/7qfz7dzkx8/ONSM-S-A0003585147-1.pdf}, 閲覧日 2022年6月27日.
	...
	\bibitem{taiyo} 太陽光発電協会, ``太陽光発電の基礎知識について'', \url{https://www.jpea.gr.jp/knowledge/about/}, 閲覧日 2022年7月5日.
	\bibitem{ondo} コベルコビジネスパートナーズ株式会社, ``結晶シリコン太陽電池による発電の実際'', \url{https://www.src-kobelco.co.jp/517-2/}, 閲覧日 2022年7月5日.
	\bibitem{mppt1}	一般社団法人日本電気工業会, ``最大電力点追従制御(MPPT)'', \url{https://www.jema-net.or.jp/Japanese/res/dispersed/data/s01.pdf}, 閲覧日 2022年7月5日.
	\bibitem{mppt2} 竹内一平 金井康通 黒川浩助, ``太陽光端セル昇圧回路へのMPPT制御の適用'', \url{http://www.kurochans.net/paper/02F_kikai_takeuchi.pdf}, 閲覧日 2022年7月5日.
\end{thebibliography}
