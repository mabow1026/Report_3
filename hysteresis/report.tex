...
\begin{enumerate}[1)]
	...
	\item 入力電圧が80Vと100Vの時はオシロスコープで波形を観測し、USBに保存した。なお、オシロスコープのCH2は反転モードに設定した。
\end{enumerate}
...
\begin{enumerate}[1)]
	...
	\item	スライダックの電圧を0Vに設定し、オシロスコープのプローブをそれぞれ次のように接続した。
	      \begin{itemize}
		      ...
		      \item CH2のプローブのGNDを図\ref{fig:circuit_int}E点
...
\end{equation}
ここで、P:ヒステリシス損、H:1DIVあたりの磁界、B:1DIVあたりの磁束密度、A:磁気回路の断面積、l:時期回路の長さ、f:電源の周波数
電源電圧が80Vのときのヒステリシス損は次のようになる。
\begin{align}
	P & = HBS \cdot Al \cdot f \nonumber                                                                                                                   \\
	  & = [\mathrm{A/m/DIV}] \times [\mathrm{Wb/m^2/DIV}] \times [\mathrm{DIV^2}] \times [\mathrm{m^2}] \times [\mathrm{m}] \times [\mathrm{Hz}] \nonumber \\
	  & \simeq [\mathrm[W]]
\end{align}
