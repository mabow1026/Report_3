...
\begin{table}[htbp]
	...
	\label{tab:実験結果}
\end{table}
表３は、励磁電流及び電力の測定結果である。それぞれの位相差$\theta[\mathrm{deg}]$は次の式を用いて導出した。
\begin{equation}
...
8頁の図7と図8は、電源電圧が$80\,\mathrm{V}$と$100\,\mathrm{V}$のときの励磁電流$i_0$と電圧$v$の関係を示したグラフである。
どちらの電圧の場合も、電圧に対して励磁電流がひずみ、位相が遅れていることがわかる。

\subsection{電流と磁束と磁束密度のグラフ(ヒステリシス曲線)}
9頁の図9と図10は、電源電圧が$80\,\mathrm{V}と100\,\mathrm{V}$のときの測定電圧$V_{\mathrm{ch1}}$と$V_{\mathrm{ch2}}$より、電流$i_0$と磁束$\phi$、
磁界$H$と磁束密度$B$の関係を表したグラフである。なお、それぞれの単位換算は図9右側の式で行った。
どちらの電圧の場合も、$V_{\mathrm{ch1}}$と$V_{\mathrm{ch2}}$、$\phi$と$i_0$が非線形の関係になっていることがわかる。
...
加えて、復元した波形と7と図8を比較すると、ほぼ同じような波形であることがわかる。


\subsubsection{ヒステリシス曲線の面積の算出}
\label{sec:menseki}
図11と図12のヒステリシス曲線で囲まれた面積をImageJを用いて計測した。今回は$1\,\mathrm{DIV}=1\,\mathrm{cm}$とした。図11のヒステリシス曲線の面積は
...
\end{equation}
ここで、$P\,[\mathrm{W}]$はヒステリシス損、$H\,[\mathrm{A/m/DIV}]$は1DIVあたりの磁界、$B[\mathrm{Wb/m^2/DIV}]$は1DIVあたりの磁束密度、$S[\mathrm{DIV^2}]$は\ref{sec:menseki}で算出した面積、$A[\mathrm{m^2}]$は磁気回路の断面積、$l[\mathrm{m}]$は時期回路の長さ、$f[\mathrm{Hz}]$は電源の周波数とする。\\\\
...
磁性体内の磁束が変化すると、磁性体内部の磁気双極子モーメントの向きが変化することで摩擦熱が発生する。\cite{henatsuki}
...
$B_m \geq 1 \mathrm{T}$のとき、$\omega_\mathrm{h}$は次の式で求められる。\cite{henatsuki}
\begin{align}
	\omega_\mathrm{h} & = \sigma_\mathrm{h} f {B_m}^2	\nonumber \\
	                  & =k_1 \frac{E^2}{f}\,[\mathrm{W/kg}]
	\label{eq:ヒステリシス損}
\end{align}
ここで、$E\,[\mathrm{V}]$を起電力、$f\,[\mathrm{Hz}]$を周波数、$\sigma_h$をヒステリシス損係数、$k_1$を比例定数とする。 \\
式(\ref{eq:ヒステリシス損})より、使用する電圧が２倍になると仮定するとき、ヒステリシス損は４倍になることがわかる。
...
\begin{align}
	...
	\label{eq:渦電流損}
\end{align}
ここで、$t\, [\mathrm{mm}]$:積層銅板一枚の厚さ、$\sigma_\mathrm{e}$は渦電流損係数、$k_2$は比例定数	\\
...
\subsubsection{電力計を使用したときの測定損失とヒステリシス損失の比較}

\begin{table}[htbp]
	\centering
	\caption{測定電力とヒステリシス損失の比較}
	\begin{tabular}{lll}
		入力電圧$V\,[\mathrm{V}]$ & 測定電力$W_0 \,[\mathrm{W}]$ & ヒステリシス損失$W_\mathrm{h}\,[\mathrm{W}]$ \\ \hline \hline
		80                    & 1.0                      & 1.85                                 \\ \hline
		100                   & 1.6                      & 2.94                                 \\ \hline
	\end{tabular}
	\label{tab:hikaku}
...
実験で測定した電力とヒステリシス曲線から求めた損失を比較したものを表\ref{tab:hikaku}に示す。
表\ref{tab:hikaku}の測定電力は回路内の全体で消費される有効電力である。従って、測定電力はヒステリシス損より大きな値をとるはずである。
しかし、実際には、式\ref{eq:80v}及び\ref{eq:100v}で算出された値よりも、測定電力のほうが小さくなっている。
このような矛盾が生じた理由として、実験で生じた誤差によるものが挙げられる。トランスから漏れた磁界によって発生した渦電流が原因で発生する渦電流損により、本来トランスで消費されるはずの電力が渦電流損として消えてしまっているのではないかと考えた。
...
\begin{itemize}
	...
	\item 変圧器における励磁電流、電力、位相差の変化を観測する。
\end{itemize}
の三点を目的として実験を行い、すべての目的を達成することができた。
